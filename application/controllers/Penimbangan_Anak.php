...
class Penimbangan_Anak extends CI_Controller
{
					...
					if ($usia >= 6 && $usia <= 8) {
							...
							switch ($usia) {
									case 6:
											...
											$id_mpasi = isset($mpasi['id_mpasi']) ? $mpasi['id_mpasi']: '';
											$kode = (isset($mpasi['kode']) ? $mpasi['kode']: '');
											$menu = (isset($mpasi['keterangan']) ? $mpasi['keterangan']: '');
											break;
    ...
    function data_penimbangan()
    {
		...
		public function update_method() {
			...
			$this->Penimbangan_model->update('penimbangan', $respData, array('id_penimbangan' => $id_penimbangan));
			$rmpasi = $this->Penimbangan_model->recommendMPSIById($id_penimbangan);
			if (sizeof($rmpasi) > 0) {
				$this->Mpasi_model->update('rekomendasi_mpasi', $data, array('id_penimbangan' => $id_penimbangan));
			} else {
				$a = $this->anak_model->edit('anak', array('id_anak' => $this->input->post('id_anak')))->row_array();
				$b = $this->anak_model->edit('ibu', array('id_ibu' => $this->input->post('id_ibu')))->row_array();
				$data = [
						'id_mpasi' => $id_mpasi,
						'id_anak' => $this->input->post('id_anak'),
						'id_penimbangan' => $id_penimbangan,
						'nama_ortu' => $b['nama_ibu'],
						'tb' => $this->input->post('tb'),
						'bb' => $this->input->post('bb'),
						'jk' => $a["jenis_kelamin"],
						'umur' => $usia,
			...
			redirect('Penimbangan_Anak/data_penimbangan');
		}
